    ...
    try {
      ...
      const updatedUser = await userModel
        .findByIdAndUpdate(
          id,
          {
            selectedAddress: saveAddress._id,
      ...
      res
        .status(500)
        .json({ success: false, message: "Unable to add new address" });
    }
    return;
  }
    ...
    try {
      ...
      if (updatedUser.selectedAddress.equals(query.delete)) {
        updatedUser = await userModel
          .findByIdAndUpdate(
            id,
            { selectedAddress: updatedUser._id },
            { new: true }
          )
          .populate("addresses")
          .populate("selectedAddress");
...
export const getUserAddressById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send({ success: false, message: "Unable to get address", error: error });
  }
};
