...
export const sendResetMail = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      res
        .status(500)
        .json({ success: false, message: "No User with that email id." });
      return;
    }
    ...
    res.status(500).json({
      success: false,
      message: "Unable to send reset mail.Please try after sometime.",
    });
  }

  res.status(200).send({
    success: true,
    message: "Password reset mail sent successfully.Please,check you inbox.",
  });
};
...
export const resetPassword = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      res.status(500).json({
        success: false,
        message: "Provided password reset token is incorrect.",
      });
      return;
    }
